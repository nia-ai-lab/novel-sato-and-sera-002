...
- サトーはシリーズを通してセラの存在を認識しない。売り子や係員などに化けたセラと短い事務的応対が起きる場合でも、その接触を関係性や印象的な交流に育てない。
- サトーパートでは敵がサトーへ直接話しかけない。誘導や工作は会話ではなく、視界の端の人の動き、設備の乱れ、物音、配置の違和感としてだけ届かせる。
- サトーの独白や台詞では、「誰にも邪魔させない」「守り抜く」「排除する」のような防衛主体の言い回しを使わせない。サトーは静かさ、いつもどおり、余計なことのなさとして受け取り、それで十分だと感じる側に留める。
- 各サトーパートでは、サトー本人の直接セリフを最低 2 つ入れる。1 つは異変を楽観的に流すためのセリフ、もう 1 つはその場の日常の快さや楽しさを言葉にするセリフで、できる限り別々に置く。
- サトーとセラの直接セリフは、Markdown 上で必ず独立段落にする。セリフの前後には空行を 1 行ずつ入れ、地の文と同じ段落へ混ぜない。
- セラの敵制圧は非致死かつ非刃物に限る。ナイフ、短剣、刃、剣、刃付きツールは使わず、必要な道具は非刃物の拘束具、解除具、通信阻害具、環境物などに限る。
- サトーの認識や独り言はサトーパートにだけ置く。セラパートでサトーの気づきや解釈を代弁しない。
- セラのセリフは、敵への呼びかけ、宣告、叱責には使わない。防衛処理の最中は基本的に台詞を置かず、必要なら地の文だけで処理する。
- セラパートでセラが喋る場合は、原則として最後の一言に限る。その一言は、防衛後に「サトーへ何をいつもどおり残せたか」を短く独白する形にする。
- セラパートの締めの一言は「何を排除したか」ではなく「サトーに何を崩さず残すか」で終える。作戦報告や勝利宣言で締めない。
- 締めの言い回しは、`〜に余計な段取りは混ぜません`、`〜以外、持たせません`、`〜だけ残れば十分です` のように、サトー側へ残る感触を言う文を優先する。
